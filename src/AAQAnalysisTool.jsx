...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .logo-box {
    ...
    font-family: var(--font-head); font-weight: 800; font-size: 18px; color: #000;
    flex-shrink: 0;
  }
  .header-text h1 {
  ...
  /* ── Content ── */
  .content { padding: 36px; max-width: 1400px; margin: 0 auto; }
  ...
  .metric-label { font-size: 11px; color: var(--muted); letter-spacing: .8px; text-transform: uppercase; }
  .metric-value { font-family: var(--font-head); font-size: 42px; font-weight: 800; color: var(--text); margin-top: 4px; }
  ...
  .badge-HIGH   { background: rgba(255,61,113,.15); color: var(--high); border: 1px solid rgba(255,61,113,.3); }
  .badge-MEDIUM { background: rgba(255,170,0,.12); color: var(--med);  border: 1px solid rgba(255,170,0,.3); }
  .badge-LOW    { background: rgba(0,229,255,.1);  color: var(--low);  border: 1px solid rgba(0,229,255,.3); }
...
const INSTANCE_DATA = [
  ...
  { instance_type: "c5.xlarge",   vCPU: 4,  memory: 8,   price_usd: 0.170  },
  { instance_type: "c5.2xlarge",  vCPU: 8,  memory: 16,  price_usd: 0.340  },
  { instance_type: "c5.4xlarge",  vCPU: 16, memory: 32,  price_usd: 0.680  },
...
function aggregateFirewall(rows) {
  ...
  for (const r of rows) {
    const k = key(r);
    if (!map[k]) {
      map[k] = { ...r, destination_port: new Set(), netstat_count: 0 };
    }
...
function perServerFirewall(rows) {
  ...
  for (const r of rows) {
    const k = key(r);
    if (!map[k]) map[k] = { server: r.source_hostname, destination_application: r.destination_application,
                             source_port: r.source_port, netstat_count: 0, destination_ports: new Set() };
    map[k].netstat_count += r.netstat_count;
    if (r.destination_port) map[k].destination_ports.add(r.destination_port);
  }
  return Object.values(map).map(r => ({
    server: r.server, destination_application: r.destination_application,
...
function DatabasesSection({ databases, firewall }) {
  ...
  return (
    <>
      ...
      <div className="filter-row">
        <input className="filter-input" placeholder="Search databases…" value={search}
    ...
    <>
      ...
      <div className="filter-row">
        <input className="filter-input" placeholder="Search…" value={search}
          onChange={e => setSearch(e.target.value)} />
        <select className="filter-select" value={usageFilter}
        ...
        <select className="filter-select" value={view}
      ...
      <div className="app">
        ...
        <div className="header">
          ...
          <div className="header-text">
            ...
            <p>KYNDRYL CLOUD MIGRATION ASSISTANT</p>
          </div>
        ...
        {result && (
          <>
            <div className="nav-bar">
              ...
              <button className="nav-btn" style={{ marginLeft: "auto", color: "var(--accent2)" }}
            ...
            <div className="content">
              {nav === "home"      && <HomeSection servers={result.servers} databases={result.databases}
                                         firewall={result.firewall} setNav={setNav} />}
              {nav === "sizing"    && <SizingSection servers={result.servers} />}
              {nav === "databases" && <DatabasesSection databases={result.databases} firewall={result.firewall} />}
              {nav === "firewall"  && <FirewallSection firewall={result.firewall} />}

              {/* Parser notes always shown */}
